...
Future<void> main() async {
  ...
  final files =
      await dir.list(recursive: true).where((entity) => entity is File).map((entity) => entity as File).toList();
  final coverage = await parseCoverage(files, 0);

  final formatter = LcovFormatter(resolver, reportOn: ['lib', 'test'], basePath: '.');

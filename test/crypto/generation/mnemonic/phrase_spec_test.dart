  ...
  group('Phrase Spec', () {
    test('random entropy (of the correct length) should be a valid phrase', () async {
      for (var i = 0; i < 10; i++) {
        ...
        final phrase = await Phrase.fromEntropy(entropy: entropy, size: size, language: English());
    ...
    test('12 phrase mnemonic with valid words should be valid', () async {
      final phrase = "cat swing flag economy stadium alone churn speed unique patch report train";
      final mnemonic = await Phrase.validated(words: phrase, language: English());
    ...
    test('12 phrase mnemonic with invalid words should be invalid', () async {
      final phrase = "amber glue hallway can truth drawer wave flex cousin grace close compose";
      final mnemonic = await Phrase.validated(words: phrase, language: English());
    ...
    test('12 phrase mnemonic with valid words and invalid checksum should be invalid', () async {
      final phrase = "ugly wire busy skate slice kidney razor eager bicycle struggle aerobic picnic";
      final mnemonic = await Phrase.validated(words: phrase, language: English());
    ...
    test('mnemonic with extra whitespace is valid', () async {
      final phrase = "vessel ladder alter error  federal sibling chat   ability sun glass valve picture";
      final mnemonic = await Phrase.validated(words: phrase, language: English());
    ...
    test('mnemonic with extra whitespace has same value as single spaced', () async {
      final phrase1 = "vessel ladder alter error federal sibling chat ability sun glass valve picture";
      final phrase2 = "vessel ladder alter error  federal sibling chat   ability sun glass valve picture";

      final mnemonic1 = await Phrase.validated(words: phrase1, language: English());
      final mnemonic2 = await Phrase.validated(words: phrase2, language: English());

...
void main() {
  ...
  final chainPrvSigning =
      Bip32SigningKey.fromValidBytes(Uint8List.fromList(chainPrv));
  final chainPubVerify = Bip32VerifyKey(Uint8List.fromList(chainPub));

  group('key generation - ', () {
    test('mnemonic to entropy', () {
      final entropy = mnemonicToEntropy(testMnemonic1);
    ...
    chainPairs.forEach((keypair) {
      test('m/1852\'/7091\'/0\'/0/$idx', () {
        final xprv = Uint8List.fromList(keypair['xprv']!);
        final xpub = Uint8List.fromList(keypair['xpub']!);
        final k = Bip32SigningKey.fromValidBytes(xprv);
        final K = Bip32VerifyKey(xpub);

...
sealed class WalletApiDefinition {
  ...
  ///
  /// [keyPair] - The Main Key Pair to derive the child key pair from.
  /// [xParty] - The first path index of the child key pair to derive. Represents the party index.
  ...
  KeyPair deriveChildKeysPartial(KeyPair keyPair, int xParty, int yContract);
  ...
  ///
  /// Returns an [Either] with a [WalletApiFailure] on the left if unsuccessful, or a [KeyPair] on the right.
  Future<Either<WalletApiFailure, KeyPair>> loadAndExtractMainKey(List<int> password, {String name = defaultName}) async {
    try {

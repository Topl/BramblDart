...
class AssetCode {
  ...
  AssetCode(this.assetCodeVersion, this.issuer, this.shortName, this.networkPrefix);
  ...
  factory AssetCode.initialize(int version, ToplAddress issuer, String name, String networkPrefix) {
    if (!isValidNetwork(networkPrefix)) {
      throw ArgumentError('Invalid network provided');
    }
    assert(version == supportedAssetCodeVersion, 'AssetCode version required to be 1');
    assert(name.length <= shortNameLimit, 'Asset short names must be no greater than 8 Latin-1 encoded characters');
    final latin1Name = Latin1Data.validated(name.padRight(shortNameLimit, latin1.decode([0])));
    final validationResult = validateAddressByNetwork(networkPrefix, issuer.toBase58());
  ...
  factory AssetCode.deserialize(String from) {
    ...
    return AssetCode(
        ...
        ToplAddress(decoded.sublist(1, 1 + ToplAddress.addressSize),
            networkId: decoded[1], proposition: PropositionType.fromPrefix(decoded[2])),
  ...
  String serialize() {
    ...
    final concatValues =
        version + slicedAddress + shortName.value!; // add trailing zeros, shortname must be 8 bytes long
  ...
  factory AssetCode.fromJson(String assetCode) => AssetCode.deserialize(assetCode);
  ...
  @override
  int get hashCode => assetCodeVersion.hashCode ^ issuer.hashCode ^ shortName.hashCode ^ networkPrefix.hashCode;
}
